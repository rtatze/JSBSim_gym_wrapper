...
from services import pid_regler_easy

if __name__ == "__main__":
    pid = pid_regler_easy.PID_regler()
    ...
    def plot(df):
        ...
        p.line(df['h'].index.values, df['h'], line_width=2,
               legend_label='h', name='h', color="black")
    ...
    while result and sim.jsbsim.get_sim_time() <= 50:
        ...
        if i%1==0:
            ...
            sim.set_properties('fcs/elevator-cmd-norm', pid.innerLoopElevator(np.deg2rad(8), state['theta'], state['q'],
                                                                              sim.jsbsim.get_property_value(
                                                                             'fcs/elevator-cmd-norm')))
        sim.run()
        #print(np.rad2deg(state['phi']))
        df = df.append({'u': state['u'], 'phi': np.rad2deg(state['phi']), 'theta': np.rad2deg(state['theta']), 'h': state['h']-3000}, ignore_index=True)
